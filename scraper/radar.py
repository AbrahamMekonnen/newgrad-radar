    ...
    for company_slug, jobs in jobs_by_company.items():
        ...
        for row in list_rows:
            ...
            if prefs.get("push_enabled"):
                # Deep-link to the persistent Notified Jobs -> Watchlist list, so
                # tapping lands on the applyable to-do list (not the generic board
                # or a one-off view they lose after closing).
                app_url = (os.getenv("APP_URL") or "https://newgrad-radar.vercel.app").rstrip("/")
                url = f"{app_url}/applications?section=alerts&notif=watchlist"
