-- Enforce the user authorization boundary in the browser claim itself.`r`n-- Only an explicit job-card click or an enabled standing rule may execute.`r`n-- Supabase installs
   ...
   WHERE q.id = (
     SELECT id
      ...
      WHERE user_id = p_user_id
        ...
        AND authorization_source IN ('direct_click', 'standing_rule')
