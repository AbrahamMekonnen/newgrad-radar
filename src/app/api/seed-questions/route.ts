...
const CODING_QUESTIONS = [
  ...
  { title: 'Word Search II', text: 'Given an m x n board of characters and a list of strings words, return all words on the board.', difficulty: 'hard' },
  ...
  { title: 'Serialize Binary Tree', text: 'Design an algorithm to serialize and deserialize a binary tree.', difficulty: 'hard' },
  { title: 'Number of Islands', text: "Given an m x n 2D binary grid representing a map of '1's (land) and '0's (water), return the number of islands.", difficulty: 'medium' },
  ...
  { title: 'Longest Palindromic Substring', text: 'Given a string s, return the longest palindromic substring in s.', difficulty: 'medium' },
  { title: 'Trapping Rain Water', text: 'Given n non-negative integers representing an elevation map, compute how much water it can trap after raining.', difficulty: 'hard' },
  { title: 'Clone Graph', text: 'Given a reference of a node in a connected undirected graph, return a deep copy of the graph.', difficulty: 'medium' },
];
...
const SYSTEM_DESIGN_QUESTIONS = [
  ...
  { title: 'Design Video Streaming', text: 'Design a video streaming service like YouTube or Netflix.', difficulty: 'hard' },
  ...
  { title: 'Design Distributed Cache', text: 'Design a distributed caching system like Redis or Memcached.', difficulty: 'hard' },
  { title: 'Design Payment System', text: 'Design a payment processing system like Stripe or PayPal.', difficulty: 'hard' },
];
...
const BEHAVIORAL_QUESTIONS = [
  ...
  { title: 'Tight Deadline', text: 'Describe a situation where you had to deliver under a very tight deadline.', difficulty: 'medium' },
  { title: 'Feedback Received', text: 'Tell me about constructive feedback you received. How did you respond?', difficulty: 'easy' },
  ...
  { title: 'Cross-team Collaboration', text: 'Tell me about a time when you collaborated with a team outside your immediate group.', difficulty: 'medium' },
  { title: 'Difficult Decision', text: 'Tell me about a difficult technical decision you had to make. How did you approach it?', difficulty: 'medium' },
];

const OA_QUESTIONS = [
  { title: 'Maximum Path Sum', text: 'Online Assessment: Find the maximum sum path from top-left to bottom-right in a grid.', difficulty: 'medium' },
  { title: 'String Manipulation', text: 'OA: Given a string, remove the minimum number of characters to make all adjacent characters different.', difficulty: 'medium' },
  { title: 'Task Scheduler', text: 'Amazon OA: Given a list of tasks and cooling time, find the minimum time to complete all tasks.', difficulty: 'medium' },
  { title: 'Warehouse Robot', text: 'Amazon OA: A robot in a warehouse needs to collect items. Find the shortest path.', difficulty: 'hard' },
  { title: 'Music Pairs', text: 'Amazon OA: Find pairs of songs whose total duration is divisible by 60.', difficulty: 'medium' },
  { title: 'Optimal Utilization', text: 'Amazon OA: Given two arrays of [id, capacity], find pairs closest to target sum.', difficulty: 'medium' },
];
...
const LEVELS = ['new_grad', 'intern', 'junior'] as const;
...
function generateQuestions() {
  ...
  for (const company of COMPANIES) {
    ...
    for (let i = 0; i < numQuestions; i++) {
      const questionTypes = ['technical_coding', 'technical_coding', 'technical_coding', 'system_design', 'behavioral', 'oa'] as const;
...
export async function GET() {
  return NextResponse.json({
    message: 'POST to this endpoint to seed interview questions',
    usage: 'curl -X POST http://localhost:3000/api/seed-questions',
  });
}
